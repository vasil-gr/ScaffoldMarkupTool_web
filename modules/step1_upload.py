...
def render_upload_sidebar():
    """Боковая панель для шага 1 (загрузка данных)"""
    st.markdown("""
    ...
    if uploaded_file is not None:
        ...
        if uploaded_file.type == "application/zip" or uploaded_file.name.endswith('.zip'):
            try:
                if validate_zip_contents(uploaded_file): # проверяем, что в zip-архиве есть ровно 1 png и 1 json файл
                    # разорхивация изображения
                    img, img_name = extract_image_from_zip(uploaded_file) # Извлекаем изображение из архива как PIL.Image
...
def render_upload_page():
    """Основное окно для шага 1"""

    setup_step1_config()

    st.write("### Scaffold Markup Tool")
    st.write("Here you can read how to work in the program: [Help](#)")  # Заменить # на реальную ссылку
    ...
    if st.session_state.get('original_img'):
        if st.session_state.base_points is not None:
            ...
            st.success("✅ Image successfully uploaded!") # ! можно добавить проверку размера (и если размер большой или маленький сделать предупреждение)
...
def validate_zip_contents(zip_file):
    """Функция для проверки содержимого zip-архива""" # для проверки, что в zip-архиве есть ровно 1 png и 1 json файл
...
def extract_image_from_zip(zip_file):
    """Функция для извлечения изображения из zip-архива""" # возвращает PIL.Image
    with zipfile.ZipFile(zip_file) as z:
        file_list = z.namelist()
...
def load_points_from_json(zip_file):
    """Функция для извлечения json-файла из zip-файла""" # возвращает координаты точек в стиле st.session_state.base_points
    with zipfile.ZipFile(zip_file) as z:
...
def calculate_safe_scale(original_width):
    ...
    if original_width <= st.session_state.display_width:
        ...
        # Округление вниз с шагом 0.05
        initial_scale = math.floor(scale_factor / st.session_state.scale_step) * st.session_state.scale_step
        initial_scale = round(initial_scale, 2)  # Округление до 2 знаков для точности
    
    # Минимальный масштаб (округляем вверх с шагом 0.05)
    ...
    # Проверяем, не превысит ли округленное значение MAX_WIDTH
