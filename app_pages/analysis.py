...
def next_step():
    ...
    if (st.session_state.step == 1 and len(st.session_state.data["image_names"]) > 0):
...
def analysis():
    ...
    if st.session_state.sidebar_state == "expanded":
        ...
        with st.sidebar: 
            ...
            with col2:
                st.button("Next", on_click=next_step, disabled=((st.session_state.step == 1 and len(st.session_state.data["image_names"]) == 0) or (st.session_state.step == 2)))
